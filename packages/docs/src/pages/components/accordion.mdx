...
AccordionHeader renders a `button` and composes `Box`, this means you add a theme key `accordion` in your theme object and add theme variant `accordionHeader` to style your `AccordionHeader`.
...
AccordionPanel renders a `Box` and composes `Box` to provide the height animation.
